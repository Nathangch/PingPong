...
def draw_winner(winner_text, color):
    ...
    pygame.display.flip()
    pygame.time.delay(3000)async def color_selection_menu():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    if selected_index == 0:
                        mode = await mode_selection_menu()
                        if mode:
                            return mode
                    if selected_index == 1:
                        await color_selection_menu()
                    if selected_index == 2:
                        pygame.quit()
                        sys.exit()
                if event.key == pygame.K_ESCAPE:
